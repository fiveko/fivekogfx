...
(function(){function b(a,b,c,f){this.controls=[];this.name=a;this.init=function(){this.controls=[];b.call(this);this.update.call(this)};this.update=function(){var a=new Date;f.call(this);for(var b=[],k=0;k<this.controls.length;k++)b.push(parseFloat(this.controls[k].value));c.apply(this,b);b=new Date;document.getElementById("log").innerText="Elapsed: "+(b-a)+" ms"};this.add=function(a){this.controls.push(a)};this.getValue=function(a){var c=this.controls.find(function(c){return c.id===a});return"checkbox"!==
c.type?c.vlaue:c.checked};this.reset=f}function h(a){var b=document.createElement("div");b.style.padding="1.2rem";b.innerHTML='<input type="file" style="display: none;"><button data-role="browse" style="margin: 10px; width: 100px;">Browse</button><button data-role="camera" style="margin: 10px; width: 100px;">Camera</button><button data-role="reset" style="margin: 10px; width: 100px;">Reset</button><select id="cmbFilters"></select>';b.querySelector("input[type=file]").addEventListener("change",d.handleFiles);
b.querySelector("button[data-role=browse]").addEventListener("click",function(){b.querySelector("input[type=file]").click()});b.querySelector("button[data-role=camera]").addEventListener("click",d.createVideo);b.querySelector("button[data-role=reset]").addEventListener("click",function(){window.redraw()});"true"===a.getAttribute("data-hide-selector")&&(b.querySelector("#cmbFilters").style.display="none");d.canvasElement.style.maxWidth="100%";a.insertBefore(b,a.firstChild)}function g(){console.log("test");
...
e.style="max-width:420px; margin:auto;";e.appendChild(g);a.appendChild(e);return g},createSelector:function(a,b){var c={x:0,y:0,w:0,h:0},f={canvas:a,value:null};a.onmousedown=function(a){c.x=a.offsetX;c.y=a.offsetY};a.onmousemove=function(b){b.buttons&&(c.w=b.offsetX-c.x,c.h=b.offsetY-c.y,b=a.getContext("2d"),redraw(),b.beginPath(),b.rect(c.x,c.y,c.w,c.h),b.stroke())};a.onmouseup=function(a){b(c)};return f},redraw:function(){}},d={videoTimerId:null,videoElement:null,imageElement:new Image,canvasElement:document.createElement("canvas"),
